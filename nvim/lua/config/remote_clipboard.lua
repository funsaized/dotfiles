...
-- local OS clipboard when one is available, so content copied in other
-- apps remains pasteable; without a display, paste is an OSC 52 query that
-- tmux (or the terminal) answers.
--
-- Linux: wl-copy / wl-paste when a Wayland session is present.
-- macOS: pbcopy / pbpaste. There is no primary selection; + and * share
-- the same pasteboard.
--
-- Local GUI nvim (no tmux, no SSH, no herdr) returns immediately and lets
-- Neovim's built-in clipboard provider handle it.
...
local function mac_comm(pid)
  local out = vim.fn.system({ "ps", "-o", "comm=", "-p", tostring(pid) })
  if vim.v.shell_error ~= 0 then
    return ""
  end
  return (out:gsub("%s+$", ""):match("[^/]+$")) or ""
end
...
local function ancestor_process_named(name)
  ...
  for _ = 1, 16 do
    ...
    if is_mac then
      ...
      comm = proc_lines(ppid, "comm")[1] or ""
    end
...
function M.setup()
  ...
  local has_local_clipboard = has_wayland or has_pbcopy
  ...
  local function paste(register)
    ...
    return function()
      ...
      if has_wayland then
        ...
        lines = vim.fn.systemlist(cmd, "", 1)
      else
        lines = vim.fn.systemlist({ "pbpaste" }, "", 1)
      end
